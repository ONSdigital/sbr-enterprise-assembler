...
import model._
import util.configuration.{AssemblerConfiguration, AssemblerOptions, Config}
import utils.Paths
...
class TestCalculations extends Paths with WordSpecLike with Matchers
  with BeforeAndAfterAll with ExistingData with ExpectedDataForAddNewPeriodScenario with ExpectedCalculations with HFileTestUtils {
