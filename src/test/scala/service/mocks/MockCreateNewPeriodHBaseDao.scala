...
import dao.hbase.HBaseDao.{entsTableName, linksTableName, lousTableName}
...
object MockCreateNewPeriodHBaseDao  {
   ...
   def readTableWithKeyFilter(confs: Configuration, tableName: String, regex: String)
    ...
    val res = tableName.split(":").last match {
      ...
      case AssemblerConfiguration.HBaseLocalUnitsTableName => readLouWithKeyFilter(confs, regex)
