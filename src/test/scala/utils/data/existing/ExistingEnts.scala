...
trait ExistingEnts{this:TestIds =>
  ...
  val existingEntsForNewPeriodScenario = Seq(
      HFileRow(s"${entWithMissingLouId.reverse}",List(KVCell("address1","P O BOX 22"), KVCell("address2","INDUSTRIES HOUSE"), KVCell("address3","WHITE LANE"), KVCell("address4","REDDITCH"), KVCell("address5","WORCESTERSHIRE"), KVCell("entref","9900000009"), KVCell("ern",entWithMissingLouId), KVCell("legal_status","1"), KVCell("name","INDUSTRIES LTD"), KVCell("paye_empees","2"), KVCell("paye_jobs","4"), KVCell("postcode","B22 2TL"), KVCell("sic07","12345"), KVCell("trading_style","A"))),
