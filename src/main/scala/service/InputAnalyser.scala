...
object InputAnalyser extends RddLogging{
  ...
  def getData(appconf:AppParams)(implicit spark: SparkSession):DataReport =  {
    ...
    val lusRdd: RDD[HFileRow] = getRepartionedRdd(HBaseDao.readLinksWithKeyFilter(Configs.conf,appconf, ".*~LEU~"+{appconf.TIME_PERIOD}+"$")
                                                 .collect{ case row if(row.cells.find(_.column == "p_ENT").isDefined) => row })

    //printRdd("LEU",lusRdd,"HFileRow")

    val losRdd: RDD[HFileRow] = getRepartionedRdd(HBaseDao.readLouWithKeyFilter(Configs.conf,appconf, ".*~"+{appconf.TIME_PERIOD}+"~.*"))
