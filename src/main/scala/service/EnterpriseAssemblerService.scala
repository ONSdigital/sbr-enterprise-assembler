...
//do Enterprises
  ...
  def createSingleRefreshHFile(appconf:AppParams) = withSpark{ implicit ss:SparkSession => withHbaseConnection { implicit con: Connection =>
    ...
    val cleanRecs: RDD[(String, hfile.HFileCell)] = HBaseDao.readWithKeyFilter(localConfigs,appconf, ".*(?<!~ENT~" + {appconf.TIME_PERIOD} + ")$").flatMap(_.toDeleteHFileRows(appconf.HBASE_LINKS_COLUMN_FAMILY)).sortBy(v => {
