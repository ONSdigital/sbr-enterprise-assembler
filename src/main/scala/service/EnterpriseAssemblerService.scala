...
import executors.CreateClosures._
/**
  *
  */
trait EnterpriseAssemblerService extends HBaseConnectionManager with SparkSessionManager{
  ...
  def loadFromJson(appconf:AppParams)(implicit ss:SparkSession){
  ...
  def loadFromParquet(appconf:AppParams){
    ...
    withHbaseConnection { implicit con: Connection => loadFromParquet(appconf) }
  }}
