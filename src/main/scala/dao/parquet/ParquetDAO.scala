...
object ParquetDAO extends WithConversionHelper with DataFrameHelper{
  ...
  def parquetToHFile(implicit spark:SparkSession,appconf:AppParams){
    ...
    val parquetRDD: RDD[hfile.Tables] = finalCalculations(spark.read.parquet(appconf.PATH_TO_PARQUET), spark.read.option("header", "true").csv(appconf.PATH_TO_PAYE)).rdd.map(row => toEnterpriseRecords(row)).cache()
